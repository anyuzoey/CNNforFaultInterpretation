...
class HED(nn.Module):
    def __init__(self, sizeaftercrop):
        super(HED, self).__init__()
        #lr 1 2 decay 1 0
        self.sizeaftercrop = sizeaftercrop
...
#         self.conv5_2_down = nn.Conv2d(512, 21, 1, padding=0)
    ...
    def forward(self, x):
        # VGG
        sizeaftercrop = self.sizeaftercrop
...
#         so5_out = self.score_dsn5(conv5_1_down + conv5_2_down + conv5_3_down)
        ...
        results = [torch.sigmoid(crop(r, sizeaftercrop, sizeaftercrop)) for r in results]
...
# model = HED(sizeaftercrop=60)
# model.cuda();
# summary(model, (3, 92, 92))
